...
function clickOnFolder(folderId) { 
  ...
  //打开Folder时关闭同一层次的其他已经打开的Folder 
  if(clicked.id > 0){//根节点除外
    var forCount = clicked.parentObj.nChildren;//选中的节点的父节点下有几个子节点，循环次数
  	for(var i=0;i<forCount;i++){
  	  if(clicked.parentObj.children[i].id != folderId){//选中的节点除外
  	    if(clicked.parentObj.children[i].isOpen){//没有打开的Folder除外
  	  	  clickOnNodeObj(findObj(clicked.parentObj.children[i].id));//关闭打开的Foler
  	    }
  	  }
    }
  }
