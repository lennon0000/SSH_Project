...
public class InitDaoImpl extends BaseDaoImpl implements InitDao {
	...
	public void addInitAdmin() {
		...
		getSession().save(adminPerson);// 人

		User adminUser = new User();// 用户
		...
		// 创建系统管理员角色
		Role adminRole = new Role();// 创建一个角色
		adminRole.setName("系统管理员");
		getSession().save(adminRole);

		// 创建普通员工角色
		Role commonRole = new Role();// 创建第二个角色
		commonRole.setName("普通员工");
		getSession().save(commonRole);

		// 让管理员用户拥有系统管理员角色
		UserRoles ur1 = new UserRoles();// 建立用户，角色关系1
		ur1.setRole(adminRole);
		ur1.setUser(adminUser);
		getSession().save(ur1);

		// 让管理员用户拥有普通员工角色
		UserRoles ur2 = new UserRoles();// 建立用户，角色关系2
		...
		// hql =
		// "select ar from ActionResource ar where ar.sn = 'security' or ar.sn = 'party'";
